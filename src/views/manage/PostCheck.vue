<template>
  <div class="post-check">
    <el-form :inline="true" label-position="left" class="form">
      <!--      <el-form-item label="审批状态" class="label-text">-->
      <!--        <el-select v-model="approvedAll" style="width: 200px;">-->
      <!--          <el-option v-for="(item, index) in isApprovedAll" :value="index" :key="index" :label="item" class="option">-->
      <!--          </el-option>-->
      <!--        </el-select>-->
      <!--      </el-form-item>-->
      ...
      <el-form-item class="label-text">
        <div class="search-box">
          <el-input
              ...
              clearable
              autofocus
              @keyup.enter="onSearchPost(inputSearch, 0)"
          >
            <!--        <template #prepend>-->
            ...
            <template #append>
              <el-button
                  :icon="Search"
                  @click="onSearchPost(inputSearch, 0)"
                  class="search-buttom"
              />
            </template>
          </el-input>
...
import {
  ...
  typeAll,
  typeSearch,
  inputSearch, onSearchPost, partNum
} from "@/stores/search"
...
function formatIsAudit (row: Post) {
  ...
  else if (row.IsAudit === 2) return "warning";
  else return "info";
}
...
//   await fetchPosts();
// });
watch([pageSize, pageNum, partNum], fetchPosts, { immediate: true });
</script>
